...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/akgcoin/akg-core>";
    const std::string URL_WEBSITE = "<https://akgblockchain.com>";

    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), "NOW", COPYRIGHT_YEAR) + " ") + "\n" +
           "\n" +
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if akgcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if akgcoin is started while in a temporary directory.\n",

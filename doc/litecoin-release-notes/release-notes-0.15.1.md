AKGcoin Core version *0.15.1* is now available from:

  <https://download.akgcoin.org/akgcoin-0.15.1/>
...
A number of changes to the way AKGcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 AKGLite per kB (0.00001 AKG). This is to prepare for dropping the minimum transaction fee to 0.1 AKGLite per kB (0.0001 AKG) in the next release.
